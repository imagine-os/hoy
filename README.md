...
- Version: **0.6.2** (2026-09-17) — **Especiales** (0017): `special_charges` + `space_bookings`, the hand-priced Especial item in S-04 with an optional teacher payout and room window, S-05 `/staff/rooms` (rooms × hours, conflict check), manual payroll lines through `payrollCalc.draftLinesFor()`, website copy for private events and special requests. Before it: v0.6.0's public website rebuilt around the brand content (W-01…W-08, P-01), the five thin screens deepened (M-02a…M-02d content CMS, M-09a/M-09b teacher payouts, S-03 payroll, A-06 as a versioned legal library, photo and map slots on one `media_assets` contract), and the operations manual rebuilt as 28 bilingual chapters in seven parts with 64 figures and live data blocks; plus, in 0.6.1, the expenses ledger (M-09c and the Balance card on M-09); 46 tables with per-table access intent; **91 routes, 79 page codes, 0 stubs** (`/#/dev/specs`), 379 screenshots, 56 components in D-02. Changelog: [`docs/changelog/`](./docs/changelog/) · what is still mocked and what is left: [`ROADMAP.md` §F](./ROADMAP.md).
