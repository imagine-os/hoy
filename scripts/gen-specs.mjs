...
const ES_NAME = {
  ...
  'C-14 / C-15': 'Preguntas frecuentes', 'C-14': 'Preguntas frecuentes (1/2)', 'C-15': 'Preguntas frecuentes (2/2)', 'C-02b': 'Horario de clases · semana', 'C-16': 'Invita a un amigo', 'C-17': 'Tarjeta de regalo',
...
const ES_PURPOSE = {
  ...
  'C-14': 'Página 1 de las preguntas frecuentes: primera vez, reservas y planes; la última parada antes de escribir a recepción.',
  'C-15': 'Página 2 de las preguntas frecuentes: el espacio, cómo llegar y contacto; termina con una persona, nunca en un callejón sin salida.',
  'C-02b': 'La vista semanal de C-02: cuadrícula de seis columnas con chips de hora; mismos datos, filtros y estado de reserva que Hoy.',
...
// Compatibility aliases for codes that canvas v1.5 retired or split (see reference/canvas/CANVAS-AUDIT.md #1, #8).
// Routes that still reference the old key keep building; drop an alias once no module uses it.
const COMPAT = {
  'C-07b': { from: 'C-07', name: { es: 'Créditos y paquetes (retirado)', en: 'Credits & packs (retired)' }, note: 'C-07b merged into C-07 in canvas v1.5; alias kept for the /app/credits stub. Content lives in C-07 + P-01.' },
  'C-14 / C-15': { from: 'C-14', name: { es: 'Preguntas frecuentes (1/2 + 2/2)', en: 'FAQ (1/2 + 2/2)' }, note: 'Shared code split into C-14 and C-15 in canvas v1.5; alias kept for the /app/faq stub. Use C-14 / C-15 separately.' },
};
...
for (const [code, alias] of Object.entries(COMPAT)) {
  if (src[code]) continue; // the canvas grew the code back; no alias needed
  const base = src[alias.from];
  const spec = {
    code, name: alias.name, purpose: { es: ES_PURPOSE[code] ?? base.intent, en: base.intent },
    layout: layout(base.layers), data: base.data ?? [], roles: roles(base.roles), logic: base.rules ?? [], integrations: integrations(base),
    states: base.states, notes: [alias.note], canvasRef: `reference/canvas/Hoy Wellness System.dc.html#${alias.from}`,
  };
  for (const k of Object.keys(spec)) if (spec[k] === undefined) delete spec[k];
  out += `  ${JSON.stringify(code)}: ${JSON.stringify(spec, null, 2).replace(/\n/g, '\n  ')},\n`;
}
out += `};\n\n/** Codes that exist in the canvas (compatibility aliases excluded). */\nexport const canvasCodes = Object.keys(canvasSpecs).filter((c) => !${JSON.stringify(Object.keys(COMPAT))}.includes(c));\n`;
