...
export function StaffHomePage() {
  ...
  const activity: TimelineItem[] = audit.map((a) => ({ id: a.id, at: a.created_at, kind: a.action.startsWith('payment') ? 'payment' : a.action.startsWith('booking') ? 'booking' : a.action.includes('note') ? 'note' : 'system', title: a.action, meta: `${byId.get(a.actor_id ?? '')?.name ?? t('staff.home.system')} · ${a.entity}${a.entity_id ? ` · ${a.entity_id}` : ''}` }));
      ...
      <div className="grid grid-2">
        <div className="stack">
          <section className="stack-sm">
            ...
            {next
              ? <ClassCard variant="next" title={next.session.title} teacher={next.teacher?.display_name ?? ''} startsAt={next.session.starts_at} endsAt={next.session.ends_at} movement={next.modality?.movement ?? 'fluye'} booked={next.session.booked_count} capacity={next.session.capacity} cta={{ label: t('staff.home.openCheckin'), onClick: () => nav(`/staff/checkin?session=${next.session.id}`) }} />
              : <EmptyState compact title={t('staff.home.next.empty')} />}
          </section>
          <Card title={t('staff.home.todayClasses')} padding="sm">
            {today.length === 0 && <p className="muted small" style={{ padding: 12 }}>{t('staff.home.next.empty')}</p>}
