...
const APP = `${tenant.slug}app://`;
const STANDARD: { key: string; name: Bi; trigger: string; subject: Bi; body: Bi; cta: { label: Bi; href: string } }[] = [
  { key: 'booking_confirmed', name: { es: 'Reserva confirmada', en: 'Booking confirmed' }, trigger: 'booking.created', subject: { es: 'Reserva confirmada: {{class_name}}', en: 'Booking confirmed: {{class_name}}' }, body: { es: 'Hola {{first_name}},\n\nTu cupo en {{class_name}} el {{class_datetime}} con {{teacher_name}} está confirmado.\n\nLlega 10 minutos antes. Cancela hasta {{cancel_hours}} h antes sin perder el crédito.', en: 'Hi {{first_name}},\n\nYour spot in {{class_name}} on {{class_datetime}} with {{teacher_name}} is confirmed.\n\nArrive 10 minutes early. Cancel up to {{cancel_hours}} h before without losing the credit.' }, cta: { label: { es: 'Ver mi reserva', en: 'View my booking' }, href: `${APP}booking/{{booking_id}}` } },
  { key: 'receipt', name: { es: 'Recibo de pago', en: 'Payment receipt' }, trigger: 'payment.approved', subject: { es: 'Tu recibo de {{studio}} · {{invoice_number}}', en: 'Your {{studio}} receipt · {{invoice_number}}' }, body: { es: 'Hola {{first_name}},\n\nRecibimos tu pago de {{amount}} por {{product}}.\n\nFactura {{invoice_number}} · Referencia DIAN: {{dian_ref}}', en: 'Hi {{first_name}},\n\nWe received your payment of {{amount}} for {{product}}.\n\nInvoice {{invoice_number}} · DIAN reference: {{dian_ref}}' }, cta: { label: { es: 'Ver historial', en: 'View history' }, href: `${APP}history` } },
  { key: 'class_cancelled', name: { es: 'Clase cancelada', en: 'Class cancelled' }, trigger: 'session.cancelled', subject: { es: 'Tu clase de {{class_name}} fue cancelada', en: 'Your {{class_name}} class was cancelled' }, body: { es: 'Hola {{first_name}},\n\nLa clase de {{class_name}} del {{class_datetime}} con {{teacher_name}} fue cancelada. Te devolvimos {{credit_returned}} crédito.\n\nElige otra clase cuando quieras.', en: 'Hi {{first_name}},\n\n{{class_name}} on {{class_datetime}} with {{teacher_name}} was cancelled. We returned {{credit_returned}} credit.\n\nPick another class whenever you like.' }, cta: { label: { es: 'Ver horario', en: 'View schedule' }, href: `${APP}classes?date={{date}}` } },
  { key: 'waitlist_promoted', name: { es: 'Cupo liberado', en: 'Spot opened' }, trigger: 'waitlist.offered', subject: { es: 'Se liberó un cupo en {{class_name}}', en: 'A spot opened in {{class_name}}' }, body: { es: 'Hola {{first_name}},\n\nSe liberó un cupo en {{class_name}} el {{class_datetime}}. Tienes {{claim_minutes}} minutos para reclamarlo.', en: 'Hi {{first_name}},\n\nA spot opened in {{class_name}} on {{class_datetime}}. You have {{claim_minutes}} minutes to claim it.' }, cta: { label: { es: 'Reclamar cupo', en: 'Claim spot' }, href: `${APP}waitlist/{{waitlist_id}}` } },
  { key: 'class_reminder', name: { es: 'Recordatorio de clase', en: 'Class reminder' }, trigger: 'booking.t-2h', subject: { es: 'Tu clase es en 2 horas', en: 'Your class is in 2 hours' }, body: { es: 'Hola {{first_name}},\n\n{{class_name}} empieza a las {{time}} con {{teacher_name}}. Trae agua y toalla.', en: 'Hi {{first_name}},\n\n{{class_name}} starts at {{time}} with {{teacher_name}}. Bring water and a towel.' }, cta: { label: { es: 'Ver clase', en: 'View class' }, href: `${APP}booking/{{booking_id}}` } },
  { key: 'how_to_prepare', name: { es: 'Cómo prepararte', en: 'How to prepare' }, trigger: 'booking.first', subject: { es: 'Tu primera clase en {{studio}}', en: 'Your first class at {{studio}}' }, body: { es: 'Hola {{first_name}},\n\nBienvenida a {{studio}}. Llega 15 minutos antes, ven con ropa cómoda y sin comer pesado 2 horas antes. Tenemos mats, toallas y duchas.', en: 'Hi {{first_name}},\n\nWelcome to {{studio}}. Arrive 15 minutes early, wear comfortable clothes and avoid heavy meals 2 hours before. We have mats, towels and showers.' }, cta: { label: { es: 'Reglas del club', en: 'Club rules' }, href: `${APP}rules` } },
  { key: 'birthday', name: { es: 'Cumpleaños', en: 'Birthday' }, trigger: 'user.birthday', subject: { es: 'Feliz cumpleaños, {{first_name}}', en: 'Happy birthday, {{first_name}}' }, body: { es: 'Hoy es tu día. Te regalamos un pase de invitado para que vengas con alguien especial.', en: 'It is your day. Here is a guest pass so you can bring someone special.' }, cta: { label: { es: 'Invitar a alguien', en: 'Invite someone' }, href: `${APP}invite` } },
  { key: 'membership_renewal', name: { es: 'Renovación de membresía', en: 'Membership renewal' }, trigger: 'membership.t-3d', subject: { es: 'Tu membresía se renueva el {{renews_at}}', en: 'Your membership renews on {{renews_at}}' }, body: { es: 'Hola {{first_name}},\n\nTu {{plan_name}} se renueva el {{renews_at}} por {{amount}}. Si quieres pausar o cambiar, hazlo desde la app.', en: 'Hi {{first_name}},\n\nYour {{plan_name}} renews on {{renews_at}} for {{amount}}. Pause or change it from the app.' }, cta: { label: { es: 'Gestionar membresía', en: 'Manage membership' }, href: `${APP}membership` } },
  { key: 'payment_failed', name: { es: 'Pago rechazado', en: 'Payment declined' }, trigger: 'payment.declined', subject: { es: 'No pudimos procesar tu pago', en: 'We could not process your payment' }, body: { es: 'Hola {{first_name}},\n\nEl pago de {{amount}} fue rechazado ({{reason}}). Puedes reintentar o cambiar de método.', en: 'Hi {{first_name}},\n\nThe payment of {{amount}} was declined ({{reason}}). Retry or change the method.' }, cta: { label: { es: 'Reintentar pago', en: 'Retry payment' }, href: `${APP}payment-methods` } },
  { key: 'gift_card_delivered', name: { es: 'Tarjeta de regalo', en: 'Gift card' }, trigger: 'gift_card.deliver', subject: { es: '{{buyer_name}} te regaló {{studio}}', en: '{{buyer_name}} gave you {{studio}}' }, body: { es: 'Hola {{first_name}},\n\nTienes un bono de {{amount}} para usar en {{studio}}. Código: {{code}}.', en: 'Hi {{first_name}},\n\nYou have a {{amount}} voucher for {{studio}}. Code: {{code}}.' }, cta: { label: { es: 'Canjear', en: 'Redeem' }, href: `${APP}gift/{{code}}` } },
  { key: 'invite', name: { es: 'Invitación', en: 'Invitation' }, trigger: 'invite.sent', subject: { es: '{{inviter_name}} te invita a {{studio}}', en: '{{inviter_name}} invites you to {{studio}}' }, body: { es: 'Hola,\n\n{{inviter_name}} te invita a una clase en HOY. Tu pase de invitado vale hasta el {{expires_at}}.', en: 'Hi,\n\n{{inviter_name}} invites you to a class at HOY. Your guest pass is valid until {{expires_at}}.' }, cta: { label: { es: 'Aceptar invitación', en: 'Accept invitation' }, href: 'https://hoy.co/invite/{{code}}' } },
];
const SAMPLE: Record<string, string> = { studio: tenant.name, first_name: 'Mariana', class_name: 'Hot Vinyasa', class_datetime: 'jue 18 sep · 17:30', teacher_name: 'Andrés', time: '17:30', cancel_hours: '4', credit_returned: '1', amount: '$39.000', product: 'Clase de Prueba', invoice_number: 'HOY-1031', dian_ref: '—', claim_minutes: '30', renews_at: '17 oct', plan_name: 'Plan Mensual', reason: 'fondos insuficientes', buyer_name: 'Sofía', code: 'HOY-REGALO-2401', inviter_name: 'Juliana', expires_at: '30 sep', booking_id: 'bk_1', waitlist_id: 'wl_1', date: '2026-09-18' };
...
export function EmailsPage() {
  ...
  const createMissing = async () => {
    for (const s of missing) {
      const r = await data.insert<EmailRow>('email_templates', { key: s.key, name: s.name.es, trigger: s.trigger, subject: s.subject, body_mjml: JSON.stringify({ es: s.body.es, en: s.body.en, cta: s.cta }), version: 1, active: false });
