...
export default defineMeta({
  ...
  a11y: [
    ...
    { es: 'Con una fila `ready`, el alt sale de `media_assets.alt` ({es,en}) y cae en `label` si está vacío.', en: 'With a `ready` row the alt comes from `media_assets.alt` ({es,en}) and falls back to `label` when empty.' },
