...
- **v0.7.1 — Polish and code-quality pass** (`docs/changelog/0020-polish-pass.md`, prompt
  ...
  feature. **Route-level code splitting**: every module's pages live in a `pages.ts` barrel loaded through
  `src/app/lazyPage.ts` (one `<Suspense>` in `App.tsx`), the markdown under `docs/` is indexed at build time by
  `scripts/lib/docmeta.mjs` and each body is its own chunk — main chunk **2 549 kB → 796 kB**. **The UTC date-key
  bug** (`toISOString().slice(0, 10)` names tomorrow after 19:00 in Bogotá) is gone: `dateKey()` / `fromDateKey()`
  in `src/i18n/format.ts` replace 14 sites and 11 `T12:00:00` workarounds, proven by `npm run test:dates`.
  Dead code swept (13 unused exports, 3 barrels, 40 unused string keys, an orphan CSS rule, 7 `eslint-disable`
  comments for a linter that does not exist), helpers deduplicated (one COP formatter, one IVA split in
  `src/data/tax.ts`, one `waLink`, one `addDays` / `addMonths`, one `MS` constant), every shared component reads
  its strings through `useT()` (LiveBlock's ~60 inline ternaries became `manual.live.*` keys), raw CSS values
  became tokens (`--r-2xs` added to D-01), tenant facts (`dialCode`, `invoicePrefix`, structured
  `openingHours` deriving the hours sentence) moved into `src/tenant/`, `tsconfig` gained `noImplicitReturns` +
  `noImplicitOverride`. The visual audit's fixes landed with it: `$` in both languages, `.eyebrow` contrast ≥ 4.5:1,
  bilingual payment statuses / plan names / feature switches / audit titles, single active sidebar item, breadcrumb
  from the nav label, wrapping card headers, the finished-day states on W-01 / S-01, role-gated links (no more
  `/no-access` bounces), `document.title` per route, and a screenshot pass that signs in as each surface's demo user
  with real ids for the param routes. Docs: flow map regenerated by `npm run flow-map`, the two missing prompt files
  reconstructed, every count in this file and the READMEs re-measured.
...
**(c) Decisions only the owner can make** — **36** of them, deduplicated in §E of this file and live
...
18. Smaller: ~~code-split the bundle by surface~~ **DONE (0.7.1)**: every module is a lazy chunk (main 796 kB). Still open: drop the C-07b and
...
28. **Module layering**: `admin/settings.ts` readers (`useSettings` / `usePolicy` / `useContact`) → `src/tenant/`,
    `staff/people.ts` + `staff/audit.ts` + the `customer/hooks.ts` joins → `src/data/`, non-UI helpers out of
    `customer/ui.tsx`; then `LiveBlock` and `MapSlot` stop importing `modules/**`. Add an import-boundary lint rule
    once eslint exists.
29. **One `sessionPhase(s, now)` predicate** replacing the 23 inline past / live / upcoming comparisons.
30. **eslint (flat config, typescript-eslint, react-hooks) + prettier**, wired into `pages.yml` — needs its own
    changelog entry with the alternative rejected. Stricter TS afterwards: `exactOptionalPropertyTypes` (79 errors
    today), `noPropertyAccessFromIndexSignature` (154), `noUncheckedIndexedAccess` (226).
31. **Bundle, next steps**: slim `canvasSpecs.ts` (121 kB — move `layerTree` / `story` / `api` into a lazily-read side
    table for the inspector), stop shipping the 63 MB of screenshots in `dist/` (link to GitHub raw or keep one pair
    per page), load the 190 kB of string tables per surface.
32. **Visual follow-ups from the 0020 audit**: one date / time format for admin tables; bilingual names for seed
    content shown as UI (email / WhatsApp template names, `cancel_reason`, expense concepts); a dark variant of
    the yellow accent surface (Notice warn, W-01 membership card, hub notice); HUB-01 card-grid composition;
    D-02 as per-tier routes (a 51 600 px page today); `scripts/screenshots.mjs --state=` for the rail and drawer.
33. **`docs/screenshots/_before/0006/`** (13 files linked from `0006`): keep as history or delete per the rule.
...
15. `remark-gfm` (§F 19) — hygiene that pays every later item (the bundle code-split, §F 18, shipped in 0.7.1).
