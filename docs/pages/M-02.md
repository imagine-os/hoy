...
- `media_assets.slot_key` is the contract with the UI: `MediaPlaceholder({ slotKey })` in the app and
  `MediaSlot({ slotKey })` / `MapSlot({ slotKey })` on the website render the URL when the row is
  `ready` and a branded empty slot otherwise. A row can only be marked ready with a URL, and marking
  it pending again hides the asset everywhere without deleting the record.
...
- **Note.** The seed contains one `media_assets` row per known slot — **12 rows** — each with the
  brief the photographer needs:

| `slot_key` | Ratio | Kind | Where it renders |
| --- | --- | --- | --- |
| `class.hero` | 16 / 9 | photo | C-03 class detail |
| `teacher.portrait` | 4 / 3 | photo | C-18 and W-05 (the brief for all eight; each finished file goes on `teachers.photo_url`) |
| `event.cover` | 4 / 5 | photo | C-23 event |
| `studio.tour` | 16 / 9 | video | C-13 club rules |
| `site.hero` | 21 / 9 | video | W-01 hero |
| `site.about` | 4 / 3 | photo | W-02 about |
| `site.contact.map` | 16 / 9 | illustration | W-06 `MapSlot`, until a map provider is chosen |
| `site.classes.hot-yoga` | 16 / 9 | photo | W-07 list + W-08 detail |
| `site.classes.barre` | 16 / 9 | photo | W-07 list + W-08 detail |
| `site.classes.pilates` | 16 / 9 | photo | W-07 list + W-08 detail |
| `site.classes.meditacion` | 16 / 9 | photo | W-07 list + W-08 detail |
| `site.classes.respiracion` | 16 / 9 | photo | W-07 list + W-08 detail |

  The five `site.classes.*` rows were added in the 0.6.0 integration pass, when the website's new
  class pages (W-07, W-08) met the media library for the first time.
