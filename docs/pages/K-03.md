...
`/manual` and `/manual/:chapter` share the K-03 code, so the unlabelled captures above are the
**chapter** route (`03-modelo-de-valor`, the param the screenshot script fills). The **cover** —
`/manual` itself, with the reading paths and the part-by-part grid — is captured under the `cover`
label by `npm run screenshots -- '--only=/manual$' --label=cover`:

| ES · cover · desktop | EN · cover · desktop |
| --- | --- |
| ![K-03 cover es 1280](../screenshots/K-03/es-1280-cover.jpg) | ![K-03 cover en 1280](../screenshots/K-03/en-1280-cover.jpg) |
...
- `[screenshot: CODE — caption]` lines render as dashed placeholder boxes until a real capture replaces them. None remain: every chapter uses real captures (72 per language).
