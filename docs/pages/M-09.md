...
2. `RevenueByProduct` — `BarList` over `payments.plan_id`
3. `RevenueByMethod` — `BarList` over `payments.method`
4. `PayoutsSummary` — the teacher-payroll roll-up, linking to M-09a
5. `PaymentsList` — every payment in range, with the refund action and the DIAN reference
6. `InvoicesTable` — invoices in range, filtered by the status of the payment behind them
...
| `payroll_runs`, `payroll_lines` | read | the three payout tiles (owed, approved-unpaid, paid this quarter) |
