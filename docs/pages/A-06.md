...
The pass resolves `:kind` to `terms`. The **waiver** is the one kind with two versions (v1.0 in
force, v1.1 in review) and the accept action, so it is captured under the `waiver` label by
`npm run screenshots -- --only=/app/legal --label=waiver` with `:kind` temporarily set to `waiver`:

| ES · waiver · mobile | ES · waiver · desktop |
| --- | --- |
| ![A-06 waiver es 390](../screenshots/A-06/es-390-waiver.jpg) | ![A-06 waiver es 1280](../screenshots/A-06/es-1280-waiver.jpg) |
...
## Changelog
- `docs/changelog/0019-app-store-readiness.md` — privacy policy §7 names the two ways to request account deletion (C-26 in the app, W-09 public); labelled captures `*-privacy.jpg` (v0.7.0)
