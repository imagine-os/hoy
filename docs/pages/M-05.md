...
- Note: The test send writes a `message_log` row — `direction outbound`, `source manual`, `body` = the template rendered with the sample variables, `queued` during quiet hours, `payload.test = true` — excluded from the customer conversations (M-06 / S-06). The log also lists the team-only rows (`user_id` null, e.g. a teacher's substitution request).

## Changelog
- `docs/changelog/0021-crm-conversacion-y-bandeja.md` — the test send writes the unified `message_log` shape; every automated send now also appears in the member's conversation (v0.8.0)
